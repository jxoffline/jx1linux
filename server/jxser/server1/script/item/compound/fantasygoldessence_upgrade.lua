...
function Compound(arynNecessaryItemIdx, arynAlternativeItemIdx, nExParam)
	...
	if nResultDesItemIdx > 0 then
		...
		WriteLog(format("[N﹏g c蕄 Huy猲 Kim Chi Tinh]\tt礽 kho秐:%s\tnh﹏ v藅:%s\tti襫 v筺 ti猽 hao%d\tnh薾 m閠 c竔%dc蕄 Huy襫 Kim Chi Tinh th祅h c玭g", GetAccount(), GetName(), nPay, nWantLevel));
		return nResultDesItemIdx, RESULT_SUCCEED;
	else
		WriteLog(format("[N﹏g c蕄 Huy猲 Kim Chi Tinh]\tt礽 kho秐:%s\tnh﹏ v藅:%s\tti襫 v筺 ti猽 hao%d\tnh薾 m閠 c竔%dc蕄 Huy襫 Kim Chi Tinh th蕋 b筰", GetAccount(), GetName(), nPay, nWantLevel));
