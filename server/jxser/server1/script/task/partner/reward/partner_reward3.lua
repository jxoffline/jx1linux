...
Include ("\\script\\task\\partner\\master\\partner_master_main_05.lua"); --增加同伴隐藏剧情的时间获取
...
function reward_startreward3(RewardTaskId,RewardTaskLevel) 
	...
	elseif ( Uworld1238 == NowDate ) then                           --修练任务时期符合要求
		if ( Uworld1241 >= 20 ) then                                 --修练任务次数已经达到上限
	...
	elseif ( nt_getTask(1239) ~= 0 ) and ( nt_getTask(1240) ~= 0 ) then
		...
		if ( PARTNER_GetTaskValue(partnerindex, 2) == 2 ) and ( nt_getTask(1237) == 0 ) then  --当玩家为任务开始状态，而1237又已被任务完成状态的函数清空时
			if ( nt_getTask(1241) == 20 ) then
				...
				taskProcess_005_AddTimes:doTaskEntity(); --隐藏剧情修炼任务全完成天数+1
			end
