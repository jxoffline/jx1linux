...
function tbVngToolAward:Show(nSeasonID)
	...
	for i = 1, getn(tbTemp) do		
		if tbTemp[i] then
			...
			if  tbExtPointLib:GetBitValueByVersion(tbExtPoint) == 1 then			
...
function tbVngToolAward:GetAward(nSeasonID, nPart)
	...
	if (tbExtPointLib:SetBitValueByVersion(tbExtPoint, 0) ~= 1)then
		return
	end
	if (tbExtPointLib:GetBitValueByVersion(tbExtPoint) ~= 0)then
		return
	end
	 for i = 1, getn(tbAwardList) do
	 	local tbItem = tbCurActive:GetSingleItem(tbAwardList[i].nAwardID)
...
function tbVngToolAward:Active_Title(nTittleID, nExpiredTime)
