...
function main(sel)
	...
	if ndate > 0710312400 then
		Say("B∏nh Trung Thu gµ n≠Ìng nµy Æ∑ qu∏ hπn sˆ dÙng.",0)
		return 0
	end
	...
	WriteLog(format("[zhongqiu2007_kaoji]\t Date:%s\t Account:%s\t Name:%s\t GetExp:%s",GetLocalDate("%y-%m-%d %H:%M:%S"),GetAccount(),GetName(),nexp));
end