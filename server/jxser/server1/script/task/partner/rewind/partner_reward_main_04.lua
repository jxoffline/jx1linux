...
PARID_REWIND04_BOSS_01 = 43;  -- 粱猫猫
PARID_REWIND04_BOSS_02 = 44;  -- 李猩
PARID_REWIND04_BOSS_03 = 45;  -- 佩公子
PARID_REWIND04_BOSS_04 = 46;  -- 唐鱼
...
rewindProcess_004_01 = inherit(CProcess, {
	...
	checkCondition = function(self)
		...
		if GetRewindTaskState(PARID_TASK_REWIND_004)==nil then  -- 没有带同伴
			return 0;
		elseif GetMasterTaskState(PARID_TASK_MASTER_004)<=2 then -- 没有完成剧情篇的任务的
			return 5;
		elseif CheckRewindState(4)~=1 then -- 不符合日期判断条件
			return 6;
		elseif GetRewindTaskState(PARID_TASK_REWIND_004)==0 then -- 第一次开始做
			return 1;
		elseif GetRewindTaskState(PARID_TASK_REWIND_004)>=1 and GetRewindTaskState(PARID_TASK_REWIND_004)<3 then -- 已经开始做了 
			return 2;
		elseif GetRewindTaskState(PARID_TASK_REWIND_004)>=3 then -- 已经完成了
			return 3;
		end;
	end,
	...
	taskEntity = function(self, nCondition)
		if nCondition==1 or nCondition==3 then
			SelectDescribe({"<npc>{临安}的{珠宝商人丁利}近日送来了一封书信。信上说那颗镶在侍郎手臂内的宝石价值连城，乃是真正入火不化的稀世珍宝天智玉。他当年获得玉胚后，以重金请来回族匠人里一等一的高手雕制，花了整整一年的时间才研磨出两颗光润剃透的宝珠。这两颗玉珠，都是被一位王姓商人买去了，据说他隐居在一处叫百花谷的地方。那个地方我是听说过的，有门徒经过那里，说谷内剑气直冲霄汉，祥云笼罩，定然有高人居住。只是这样的事情发生以后，那个地方更显得神秘了。我料定你此行必将有所收获，不过还需做好准备啊。到了谷里找到那个叫王一的商人。",
							"接受任务/#rewindProcess_004_Accept:doTaskEntity()",
							"我再考虑一下/OnTaskExit",
							});
			return 1;
		elseif nCondition==2 then
			SelectDescribe({"<npc>你完成了我给你的任务了吗？",
							"请送我去百花谷吧/#rewindProcess_004_GotoWorld:doTaskEntity()",
							"完成任务/#rewindProcess_004_Finish:doTaskEntity()",
							"我还没有完成/OnTaskExit"
							});
			return 1;
...
rewindProcess_004_Accept = inherit(CProcess, {
	...
	taskEntity = function(self, nCondition)
		SelectDescribe({"<npc>你现在想去百花谷吗？我可以送你一城。",
						"是的，麻烦你了/#rewindProcess_004_GotoWorld:doTaskEntity()",
						"让我再准备准备吧/OnTaskExit"});
		Msg2Player("你接到了异族武士的同伴修炼任务！");
	...
	taskEntity = function(self, nCondition)
		...
		if nNum_1==1 and nNum_2==1 and nNum_3==1 and nNum_4==1 and GetRewindTaskState(PARID_TASK_REWIND_004)==1 then
			SelectDescribe({"<npc>谢谢你替我解除了危险。既然是五爷的人，我这里有一样东西交托给你。侍郎是我的好友，当初他嘱托我代为保管，如今他人也死了，这张帖子我留之无用，反是个大麻烦。你就拿去吧。对了，既然你是爱武之人，我这谷内还隐居了一位旧时名扬天下的高手，不过他戒武已久。你若能说得动他，说不定会传一门绝技予你吧。",
							"还是先将我送回去吧/#rewindProcess_004_02_SendMan:doTaskEntity()",
							"结束对话/OnTaskExit"});
...
rewindProcess_004_02_SendMan = inherit(CProcess, {
	taskEntity = function(self, nCondition)
		SelectDescribe({"<npc>你要离开这里了吗？",
						"是的，请送我走吧/#rewindProcess_004_02_Send:doTaskEntity()",
						"再等等/OnTaskExit"
						});
		return 1;
	end,
