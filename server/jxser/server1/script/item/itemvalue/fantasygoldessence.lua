-- 功能：新白金装备系统 - 玄金之精的价值量计算
-- baiyun 2012.03.16

IncludeLib( "FILESYS" );
Include( "\\script\\item\\item_header.lua" );

FANTASYGOLDESSENCE_VALUE_FILE = "itemvalue\\FantasyGoldEssence.txt";

loadItemTabFiles( FANTASYGOLDESSENCE_VALUE_FILE );
...
function CalcItemValue( nItemVer, nQuality, nGenre, nDetailType, nParticular, nLevel, nSeries, nLuck, arynMagLvl, aryMagic, strParam )
	local nItemVal = 0;
	local strOreValPath = makeItemFilePath( nItemVer, FANTASYGOLDESSENCE_VALUE_FILE );
...
--提供给脚本使用快捷获取等级对应的价值量
function CalcFantasyGoldEssenceValueByLevel(nLevel)
	local nItemVersion = ITEM_GetLatestItemVersion();
	local nItemValue = 0;
	local strValueFilePath = makeItemFilePath(nItemVersion, FANTASYGOLDESSENCE_VALUE_FILE);
