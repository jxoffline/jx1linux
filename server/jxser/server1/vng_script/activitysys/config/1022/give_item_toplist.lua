...
tbTop10.tbPhase = {
	[1] = {201601160000, 201601311159}, --nép item dua top, 12h tr­a ngµy cuèi ko co nép
	[2] = {201601311200, 201601312400}, --nhËn th­ëng tõ 12h ®Õn 24 h
}
...
function tbTop10:GiveItem(strName, strAccount, nLastCount,nCount)
	...
	if nPhase ~= 1 then
		Msg2Player(format("HiÖn t¹i kh«ng ph¶i thêi gian ®ua top %s.",ITEM_TINH_THACH.szName))
		return
	end
	...
	if self:Subcribers2File() == 1 then
		Msg2Player(format("Nép M¶nh Thiªn Th¹ch ®ua top thµnh c«ng, tæng sè: %d.",nLastCount + nCount))			
		Talk(1,"",format("Nép <color=yellow>%d<color> M¶nh Thiªn Th¹ch ®ua top thµnh c«ng, tæng sè: <color=yellow>%d<color>.",nCount, nTotal ))		
