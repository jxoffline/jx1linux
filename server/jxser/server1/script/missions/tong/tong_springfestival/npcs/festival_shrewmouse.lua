...
function main()
	...
	Msg2Player(format("§¸nh tróng %d %s", nCount, SF_MOUSENAME));
end;
	...
	for i =1, getn(ITEM_DROPRATE_TABLE) do
		...
		if (nSum >= nRandNum) then
			if (type( ITEM_DROPRATE_TABLE[i][1] ) == "table") then
				...
				Msg2Player(format("NhËn ®­îc 1 <color=0xB5FDD7>%s",ITEM_DROPRATE_TABLE[i][3]));
			elseif (type( ITEM_DROPRATE_TABLE[i][1] ) == "number") then
			    local nExp = ITEM_DROPRATE_TABLE[i][1] * festival_get_exptimes();
				AddOwnExp(nExp);
				Msg2Player(format("NhËn ®­îc <color=0xB5FDD7> ®iÓm kinh nghiÖm.", nExp))
			end
			break
		end
	end
