...
function GetNpcLevelData(Series, Level, StyleName, ParamStr)
	...
	if(StyleName=="Skill2" or StyleName=="Skill3") then 
		if(Series==0) then
			return 424
	...
	if(StyleName=="PhysicsResist") then --普抗
		if(Series==0) then
			return 2*floor(Link(Level,{{1,0},{100,25}}))
		elseif(Series==1) then
			return floor(Link(Level,{{1,0},{20,-10},{60,-15},{100,-30}}))
		elseif(Series==2) then
			return 2*floor(Link(Level,{{1,0},{100,25}}))
		elseif(Series==3) then
			return 2*floor(Link(Level,{{1,10},{20,30},{60,70},{100,95}}))
		elseif(Series==4) then
			return 2*floor(Link(Level,{{1,0},{100,25}}))
		end
		return 0
...
if (StyleName == "AttackRating") then
	return 1.5*floor(GetAttackRating(Level, Param1, Param2)*1.2);
end;

if (StyleName == "Defense") then
	return 2*floor(GetDefense(Level, Param1, Param2)*1.1);
end;

if (StyleName == "MinDamage") then
return 2*GetMinDamage(Level, Param1, Param2);
end;

if (StyleName == "MaxDamage") then
return 2*GetMaxDamage(Level, Param1, Param2);
end;
