...
function GetNpcLevelData(Series, Level, StyleName, ParamStr)
	if(StyleName=="Level1" or StyleName=="Level2" or StyleName=="Level3" or StyleName=="Level4") then
		return floor(Link(Level,{{1,1},{100,60}}))
	end;
	...
	if(StyleName=="PhysicalDamageBase") then --外普攻击
		return floor(5*Level*2/3)*3.5
	end;
	if(StyleName=="PhysicalMagicBase") then --内普攻击
		if(Series==0) then
			return floor((Link(Level,{{1,5},{20,50},{60,300},{100,600}})/2)*2/3)*3.5
		elseif(Series==1) then
			return 0
		elseif(Series==2) then
			return 0
	...
	if(StyleName=="PoisonMagicBase") then --内毒攻击
		if(Series==0) then
			return 0
		elseif(Series==1) then
			return floor((Link(Level,{{1,5},{20,50},{60,300},{100,600}})/20)*2/3)*3.5
		elseif(Series==2) then
			return 0
		elseif(Series==3) then
			return 0
...
if (StyleName == "AttackRating") then
	...
	return floor(GetAttackRating(Level, Param1, Param2)*1.5);
end;
...
if (StyleName == "Defense") then
	...
	return floor(GetDefense(Level, Param1, Param2)*2);
end;

if (StyleName == "MinDamage") then
return floor(GetMinDamage(Level, Param1, Param2));
end;

if (StyleName == "MaxDamage") then
return floor(GetMaxDamage(Level, Param1, Param2));
end;
