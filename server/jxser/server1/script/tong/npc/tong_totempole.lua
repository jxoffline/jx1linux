...
function main()
	...
	elseif (nstuntID == 0) then
			totempole_dec("Kü n¨ng bang héi ch­a më, kh«ng thÓ sö dông Cét biÓu t­îng, h·y b¸o cho cÊp l·nh ®¹o bang khai më kü n¨ng")
			return
	elseif (TONG_GetTaskValue(nTongID, TONGTSK_STUNT_PAUSE) == 0 or TONG_GetTaskValue(nTongID, TONGTSK_STUNT_PAUSE) == 1) then
		totempole_dec("Bang héi ®· ®ãng b¶o tr× kü n¨ng, hiÖn t¹i kh«ng thÓ sö dông Cét biÓu t­îng.")
		return
	elseif (TONG_GetTaskValue(nTongID, TONGTSK_STUNT_ENABLED) == 0) then
			totempole_dec("TiÒn chiÕn bÞ bang héi kh«ng ®ñ ®Ó b¶o tr× kü n¨ng nµy, kh«ng thÓ sö dông kü n¨ng bang héi!")
			return
	end
		--Ö´ÐÐ¶ÔÓ¦ÌØ¼¼
...
function IsTakeStunt(nTongID,nStuntRight,nstuntID)
	...
	elseif (TONG_GetWeek(nTongID) > TONG_GetTaskValue(nTongID, TONGTSK_STUNT_MAINPERIOD)) then
		totempole_dec("Kü n¨ng bang héi ®· qu¸ thêi h¹n b¶o tr×, hiÖn th¹i kh«ng thÓ tiÕp nhËn, sau khi b¶o tr× ngµy h«m nay h·y ®Õn thö xem.")
		return
	end
	
	local nHour = tonumber(GetLocalDate("%H"))
	if (nHour < 20 or nHour > 22) then
		totempole_dec("Thêi gian nhËn kü n¨ng bang héi lµ 20h00 ®Õn 22h00, h·y canh giê mµ ®Õn nhËn.")
		return
	end
	...
	--¹ÜÀíÌØ¼¼µÄÈ¨ÏÞ
		...
		if (nFigure == 1) then
			if (TONG_GetTaskValue(nTongID, TONGTSK_STUNT_MEMLIMIT) > 0) then
				...
				totempole_dec("Kü n¨ng bang héi trong tuÇn nµy ®· nhËn hÕt, tuÇn sau h·y ®Õn.")
			end
		else
			totempole_dec("ChØ cã cÊp l·nh ®¹o míi cã thÓ nhËn kü n¨ng bang héi nµy.")
		end
		return
	end
	...
	if (GetTask(TASKID_STUNT_LASTWEEK) ==  TONG_GetTaskValue(nTongID, TONGTSK_STUNT_MAINPERIOD) and GetTask(TASKID_STUNT_TAKED) >= TB_STUNTID_INFO[nstuntID].cntper) then
		totempole_dec("TuÇn nµy ®· nhËn kü n¨ng bang héi råi, kh«ng thÓ nhËn n÷a.")
		return
	...
	if (TONG_GetTaskValue(nTongID, TONGTSK_STUNT_MEMLIMIT) <= 0) then
		totempole_dec("H«m nay ®· nhËn kü n¨ng bang héi råi, kh«ng thÓ nhËn n÷a.")
		return
	end
...
function stunt_huangjinling(nTongID, nstuntID)
	...
	totempole_dec(
	"<enter>Kü n¨ng bang héi hiÖn t¹i: "..
	...
	"<enter><color=yellow>    "..(TB_STUNT_HUANGJINLING.consume).." v¹n tiÒn chiÕn  bÞ<color>"..
	"<enter>T¸c dông cña kü n¨ng ®Æc biÖt:"..
	"<enter><color=yellow>    "..TB_STUNT_HUANGJINLING.describe.."<color>"..
	"<enter>Thêi h¹n cña kü n¨ng nµy lµ 1 tuÇn. H«m nay cho phÐp "..TONG_GetTaskValue(nTongID, TONGTSK_STUNT_MEMLIMIT).." ng­êi n÷a ®Õn nhËn kü n¨ng nµy.", 
...
function stunt_huangjinling1()
	...
	if (AddItem(6, 1, 1107, 1, 1, 1) > 0) then
		...
		Msg2Tong(nTongID, GetName().."nhËn 1 c¸i Hoµng Kim LÖnh")
		TONG_ApplyAddEventRecord(nTongID, GetName().."nhËn 1 c¸i Hoµng Kim LÖnh")
	end
end
...
function stunt_lingshenli(nTongID, nstuntID)
	...
	"<enter>Chi phÝ b¶o tr× ®Æc kü:"..
	"<enter><color=yellow>    "..(TB_STUNT_LINGSHENLI.consume).." v¹n tiÒn chiÕn  bÞ<color>"..
	"<enter>T¸c dông cña kü n¨ng ®Æc biÖt:"..
	"<enter><color=yellow>    "..TB_STUNT_LINGSHENLI.describe.."<color>"..
	"<enter>Thêi h¹n cña kü n¨ng nµy lµ 1 tuÇn, trong vßng 1 tuÇn cho phÐp nhËn tèi ®a "..TB_STUNT_LINGSHENLI.maxmem.." ng­êi tiÕp nhËn kü n¨ng, mçi ngµy mét ng­êi chØ nhËn ®­îc 1 lÇn. H«m nay cßn cho phÐp "..TONG_GetTaskValue(nTongID, TONGTSK_STUNT_MEMLIMIT).." ng­êi n÷a ®Õn nhËn kü n¨ng nµy.", 
...
function stunt_shiweiying(nTongID, nstuntID)
	...
	local nlastDay = getRelativeDay(nTongID) - TONG_GetTaskValue(nTongID, TONGTSK_STUNT_MAINPERIOD) + 7
	totempole_dec(	"<enter>Kü n¨ng bang héi hiÖn t¹i: "..
	"<enter><color=yellow>    "..TB_STUNT_SHIWEIYING.name.."<color>"..
	"<enter>QuyÒn tiÕp nhËn kü n¨ng: "..
	"<enter><color=yellow>    "..TB_STUNT_RIGHT_DS[TB_STUNT_SHIWEIYING.right].."<color>"..
	"<enter>Chi phÝ b¶o tr× ®Æc kü:"..
	"<enter><color=yellow>    "..(TB_STUNT_SHIWEIYING.consume).." v¹n tiÒn chiÕn  bÞ<color>"..
	"<enter>T¸c dông cña kü n¨ng ®Æc biÖt:"..
	"<enter><color=yellow>    "..TB_STUNT_SHIWEIYING.describe.."<color>"..
	"<enter>Thêi h¹n cña kü n¨ng nµy lµ 1 tuÇn, trong vßng 1 tuÇn cho phÐp nhËn tèi ®a "..TB_STUNT_SHIWEIYING.maxmem.." ng­êi tiÕp nhËn kü n¨ng, mçi ngµy mét ng­êi chØ nhËn ®­îc 1 lÇn. H«m nay cßn cho phÐp "..TONG_GetTaskValue(nTongID, TONGTSK_STUNT_MEMLIMIT).." ng­êi n÷a ®Õn nhËn kü n¨ng nµy.", 
...
function stunt_moneybox(nTongID, nstuntID)
	...
	"<enter>Chi phÝ b¶o tr× ®Æc kü:"..
	"<enter><color=yellow>    "..(TB_STUNT_MONEYBOX.consume).." v¹n tiÒn chiÕn  bÞ<color>"..
	"<enter>T¸c dông cña kü n¨ng ®Æc biÖt:"..
	"<enter><color=yellow>    "..TB_STUNT_MONEYBOX.describe.."<color>"..
	"<enter>Thêi h¹n cña kü n¨ng nµy lµ 1 tuÇn, trong vßng 1 tuÇn cho phÐp nhËn tèi ®a "..TB_STUNT_MONEYBOX.maxmem.." ng­êi tiÕp nhËn kü n¨ng, mçi ngµy mét ng­êi chØ nhËn ®­îc 1 lÇn. H«m nay cßn cho phÐp "..TONG_GetTaskValue(nTongID, TONGTSK_STUNT_MEMLIMIT).." ng­êi n÷a ®Õn nhËn kü n¨ng nµy.", 
...
function stunt_wushuang(nTongID, nstuntID)
	...
	"<enter>Chi phÝ b¶o tr× ®Æc kü:"..
	"<enter><color=yellow>    "..(TB_STUNT_WSSZ.consume).." v¹n tiÒn chiÕn  bÞ<color>"..
	"<enter>T¸c dông cña kü n¨ng ®Æc biÖt:"..
	"<enter><color=yellow>    "..TB_STUNT_WSSZ.describe.."<color>"..
	"<enter>Thêi h¹n cña kü n¨ng nµy lµ 1 tuÇn.", 
...
function wsh_getwushuangshazhen()
	...
	if (nItem > 0) then
		...
		Msg2Tong(nTongID, GetName().."NhËn kü n¨ng bang héi V« song s¸t trËn")
		TONG_ApplyAddEventRecord(nTongID, GetName().."NhËn kü n¨ng bang héi V« song s¸t trËn")
	end;
...
function stunt_duanwuhun(nTongID, nstuntID)
	...
	"<enter>Chi phÝ b¶o tr× ®Æc kü:"..
	"<enter><color=yellow>    "..(TB_STUNT_DWH.consume).." v¹n tiÒn chiÕn  bÞ<color>"..
	"<enter>T¸c dông cña kü n¨ng ®Æc biÖt:"..
	"<enter><color=yellow>    "..TB_STUNT_DWH.describe.."<color>"..
	"<enter>Thêi h¹n cña kü n¨ng nµy lµ 1 tuÇn, trong vßng 1 tuÇn cho phÐp nhËn tèi ®a "..TB_STUNT_DWH.maxmem.." ng­êi tiÕp nhËn kü n¨ng, mçi ngµy mét ng­êi chØ nhËn ®­îc 1 lÇn. H«m nay cßn cho phÐp "..TONG_GetTaskValue(nTongID, TONGTSK_STUNT_MEMLIMIT).." ng­êi n÷a ®Õn nhËn kü n¨ng nµy.", 
...
function stunt_fenghuang(nTongID, nstuntID)
	...
	"<enter>Chi phÝ b¶o tr× ®Æc kü:"..
	"<enter><color=yellow>    "..(TB_STUNT_FHY.consume).." v¹n tiÒn chiÕn  bÞ<color>"..
	"<enter>T¸c dông cña kü n¨ng ®Æc biÖt:"..
	"<enter><color=yellow>    "..TB_STUNT_FHY.describe.."<color>"..
	"<enter>Thêi h¹n cña kü n¨ng nµy lµ 1 tuÇn, trong vßng 1 tuÇn cho phÐp nhËn tèi ®a "..TB_STUNT_FHY.maxmem.." ng­êi tiÕp nhËn kü n¨ng, mçi ngµy mét ng­êi chØ nhËn ®­îc 1 lÇn. H«m nay cßn cho phÐp "..TONG_GetTaskValue(nTongID, TONGTSK_STUNT_MEMLIMIT).." ng­êi n÷a ®Õn nhËn kü n¨ng nµy.", 
...
function stunt_wushenbless(nTongID, nStuntID)
	...
	"<enter>Chi phÝ b¶o tr× ®Æc kü:"..
	"<enter><color=yellow>    "..(TB_STUNT_WUSHENBLESS.consume).." v¹n tiÒn chiÕn  bÞ<color>"..
	"<enter>T¸c dông cña kü n¨ng ®Æc biÖt:"..
	"<enter><color=yellow>    "..TB_STUNT_WUSHENBLESS.describe.."<color>"..
	"<enter>Thêi h¹n cña kü n¨ng nµy lµ 1 tuÇn, trong vßng 1 tuÇn cho phÐp nhËn tèi ®a "..TB_STUNT_WUSHENBLESS.maxmem.." ng­êi tiÕp nhËn kü n¨ng, mçi ngµy mét ng­êi chØ nhËn ®­îc 1 lÇn. H«m nay cßn cho phÐp "..TONG_GetTaskValue(nTongID, TONGTSK_STUNT_MEMLIMIT).." ng­êi n÷a ®Õn nhËn kü n¨ng nµy.", 
