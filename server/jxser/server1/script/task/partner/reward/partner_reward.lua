...
Include ("\\script\\task\\partner\\master\\partner_master_main_05.lua"); 					 --增加同伴隐藏任务记录天数的需求
...
function reward_startreward(RewardTaskId,RewardTaskLevel) 
	...
	elseif ( Uworld1238 == 0 ) then                                --开始当天第一个修练任务
		...
		nt_setTask(1246,5)                                         --第一次启动任务时，将同伴每天允许完成的次数设为5
	...
	elseif ( Uworld1238 == NowDate ) then                           --修练任务时期符合要求
		if ( Uworld1241 >= 5 ) then                                 --修练任务次数已经达到上限
...
function reward_givetask()
	...
	elseif ( nt_getTask(1239) ~= 0 ) and ( nt_getTask(1240) ~= 0 ) then
		...
		if ( PARTNER_GetTaskValue(partnerindex, 2) == 2 ) and ( nt_getTask(1237) == 0 ) then  --当玩家为任务开始状态，而1237又已被任务完成状态的函数清空时
			...
			if ( nt_getTask(1242) == 5 ) and ( GetBit(GetTask(1269),1) == 0 ) then
				nt_setTask(1245,2)
				Msg2Player("恭喜你获得了青龙武士的称号！")
				Title_AddTitle(77, 1, TITLETIME);
				Title_ActiveTitle( 77 );
				nt_setTask(1269,SetBit(GetTask(1269),1,1)) 
				reward_cleartaskvalue()
			elseif ( nt_getTask(1242) == 10 ) and ( GetBit(GetTask(1269),2) == 0 )then
				nt_setTask(1245,3)
				Msg2Player("恭喜你获得了青龙剑煞的称号！")
				Title_AddTitle(78, 1, TITLETIME);
				Title_ActiveTitle( 78 );
				nt_setTask(1269,SetBit(GetTask(1269),2,1)) 
				reward_cleartaskvalue()
			elseif ( nt_getTask(1242) == 20 ) and ( GetBit(GetTask(1269),3) == 0 ) then
				nt_setTask(1245,4)
				nt_setTask(1246,10)                       --增加每天可完成修练任务次数为10次！
				Msg2Player("恭喜你获得了青龙武绝的称号！每天允许完成修练任务的次数被增加到10次了！")
				Title_AddTitle(79, 1, TITLETIME);
				Title_ActiveTitle( 79 );
				nt_setTask(1269,SetBit(GetTask(1269),3,1)) 
				reward_cleartaskvalue()
			end
		end
	else
