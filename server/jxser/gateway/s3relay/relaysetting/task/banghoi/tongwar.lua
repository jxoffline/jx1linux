...
--  ID = {[1]±¨Ãû¿ªÊ¼ÈÕ,[2]±ÈÈüÆð,	[3]±ÈÈüÖ¹,	[4]½±ÀøÆð,	[5]½±ÀøÖ¹}
...
function TaskShedule()
	...
	OutputMsg("==========================================================");
	OutputMsg("[Start] Bang héi khiªu chiÕn - "..TONGWAR_NAME.."")
...
function TaskContent()
	...
	OutputMsg(format("Mua hien tai la %s", nseason))
	if (nseason == 0) then		
		set_tongwar_phase(0);
		--OutputMsg("season "..task_month.." cannot be found in schedule, quit task."..nseason)
		return 0;
	end;
	if (task_ymd > TONGWAR_SEASON_TB[nseason][5]) then
		set_tongwar_phase(0);
	...
	if (not team1) or (not team2) or (not team3) then
		OutputMsg("date "..task_date.." cannot be found in schedule, quit task.")
		return
	end
	if (task_time >= (TONGWAR_STATETIME - 5) and task_time <= (TONGWAR_STATETIME + 5)) then
...
function tongwar_initmatch(nseason)
	...
	for strCityTongName, tbCity in tbTongSet do
		...
		if getn(tbCity) == 1 and strCityTongName ~= nil and strCityTongName ~= "" then
			...
			if FALSE(LG_GetLeagueObj(TONGWAR_LGTYPE, strCityName)) then
				...
				OutputMsg("Bang héi tham gia Vâ L©m §Ö NhÊt Bang ®· ®­îc x¸c ®Þnh!!");
			end
	...
	for i = 10225, 10234 do		
		Ladder_ClearLadder(i);
	end;
	 gb_SetTask("TONGWAR_STATION", 2,1)	
...
function set_tongwar_phase(nphase)
	if (gb_GetTask("TONGWAR_STATION", 2) ~= nphase) then
		gb_SetTask("TONGWAR_STATION", 2,nphase)	
