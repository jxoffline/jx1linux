...
function ObjBuffer:PopObject(handle)
	...
	elseif (nDataType == OBJTYPE_NIL) then
		...
		if (nDataType == nil) then
			print("load[NIL] is not supported!!!")
		else
			print("load["..nDataType.."] is not supported!!!")
		end
		return nil
	end
end
...
--指定读取某个类型的数据
function ObjBuffer:PushByType(handle, nDataType, obj)
	local szLuaType = type(obj)
	if (szLuaType == "number" and nDataType == OBJTYPE_NUMBER) then
		return ObjBuffer:PushNumber(handle, obj)
	elseif (szLuaType == "string" and nDataType == OBJTYPE_STRING) then
		return ObjBuffer:PushString(handle, obj)
	elseif (szLuaType == "table" and nDataType == OBJTYPE_TABLE) then
		return ObjBuffer:PushTable(handle, obj)
	elseif (szLuaType == "nil" and nDataType == OBJTYPE_NIL) then
		return nil
	else
		print("data type error", szLuaType, nDataType)
	end
end

--指定读取某个类型的数据
function ObjBuffer:PopByType(handle, nDataType)
	if (nDataType == OBJTYPE_NUMBER) then
		return ObjBuffer:PopNumber(handle)
	elseif (nDataType == OBJTYPE_STRING) then
		return ObjBuffer:PopString(handle)
	...
	elseif (nDataType == OBJTYPE_NIL) then
		...
		print(format("load[%s] is not supported!!!", nDataType))
		return nil
	end
end
...
function ObjBuffer:PushObject(handle, obj)
	...
	elseif (tname == "nil") then
		...
		print("save["..tname.."] is not supported!!!")
	end
end
