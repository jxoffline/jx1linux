...
tbAnBangHKL = tbBaseClass:new()

tbAnBangHKL.szPath = "\\settings\\task\\killmostert.txt"
tbAnBangHKL.TSK_TASK_ID = 2690
tbAnBangHKL.TSK_KILL_COUNT = 2691
function tbAnBangHKL:_init()
...
function tbAnBangHKL:AcceptTask(nStart, nEnd)
...
function tbAnBangHKL:IsHaveTask(bFlag)
...
function tbAnBangHKL:CancelTask()
...
function tbAnBangHKL:KillMonster(nNpcIndex)
...
function tbAnBangHKL:CheckTaskId(nStart,nEnd)
	if not self.tbTaskList then
		return 
	end
...
function tbAnBangHKL:CheckTaskComplete()
...
function tbAnBangHKL:ReviewCurKillMonsterTask()
	local nTaskId = GetTask(self.TSK_TASK_ID)
	local nKillCount = GetTask(self.TSK_KILL_COUNT)
...
function tbAnBangHKL:CompleteTask()
	SetTask(self.TSK_TASK_ID, 0)
	SetTask(self.TSK_KILL_COUNT, 0)
