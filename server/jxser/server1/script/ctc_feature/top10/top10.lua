...
Include("\\script\\ctc_feature\\top10\\top10_h.lua")
...
function tbTop10:CheckTime()
	...
	if (nTime >= 0000 and nTime <=0005) or (nTime >= 1000 and nTime <=1005) or (nTime >= 1500 and nTime <=1505) then
		return 1
	end
	return 0
end
...
function tbTop10:CheckDay()
	...
	if nTime < 20210707 or nTime > 20210731 then
		return 0
	end
	return 1
end
...
function tbTop10:IsActive()
	local nTime = tonumber(date("%Y%m%d"))
	if nTime < 20210707 or nTime > 20210830 then
		return 0
	end
	return 1
end
...
function tbTop10:Regist()
	...
	if (GetLevel() < 10)then
		Talk(1,"","CÊp 10 trë lªn míi cã thÓ ®ang ký!")
		return
	end
	...
	tinsert(tbSubcriber, strAccount)
...
function tbTop10:UpdateInfo()
	...
	if self.CheckTime() == 1 then
		...
		if (GetLevel() < 10)then
		Talk(1,"","CÊp 10 trë lªn míi cã thÓ  tham gia")
		return
	end
	...
	if nTaskDayCount > 3 then
		Talk(1, "", "Mçi ngµy chØ ®­îc cËp nhËt th«ng tin 3 lÇn, ngµy mai råi h·y quay l¹i.")
		return
	end
	local tbSubcriber = self.tbSubcribers[strName]

	--level hiÖn t¹i
	tbSubcriber[self.strAccount] = GetAccount()
	tbSubcriber[self.nCUR_LVL] = GetLevel()
	--kinh nghiÖm hiÖn t¹i
	tbSubcriber[self.nCUR_EXP] = GetExp()/1e9
	tbSubcriber[self.nCUR_EXP_PERCENT] = GetExpPercent()
	if self:Subcribers2File() == 1 then
		PlayerFunLib:AddTaskDaily(self.nTaskDayLimit, nTaskDayCount +1)
...
function tbTop10:Subcribers2File()
	...
	for key, val in self.tbSubcribers do
		...
		--tinsert(tbTemp, {key, val[self.nCUR_LVL], val[self.nCUR_EXP], val[self.nCUR_EXP_PERCENT]})
		tinsert(tbTemp, {key, val[self.strAccount],val[self.nCUR_LVL], val[self.nCUR_EXP], val[self.nCUR_EXP_PERCENT]})
	end
...
function tbTop10:ShowTopList()
	...
	--strTittle = strTittle..format("%-5s%-17s%-12s%-10s%-8s\n", "H¹ng", "Tªn", "Trïng Sinh", "CÊp", "Exp(Tû)")
	strTittle = strTittle..format("%-5s%-17s%-10s%-8s\n", "H¹ng", "Tªn", "CÊp", "Exp(tû)")
	local strTittle_TS = ""
	local strName = ""
	local strCurLVL = ""
	for nRank = 1, getn(self.tbTopList) do
		...
		if strName == GetName() then
			...
			strTittle = strTittle..format("<color=cyan>%-5s%-17s%-10s%-8.04f<color>\n", nRank, strName, strCurLVL,self.tbTopList[nRank][4])
		else
			--strTittle = strTittle..format("<color=yellow>%-5s%-17s%-12s%-10s%-8.04f<color>\n", nRank, strName, strTittle_TS, strCurLVL,self.tbTopList[nRank][4])
			strTittle = strTittle..format("<color=yellow>%-5s%-17s%-10s%-8.04f<color>\n", nRank, strName, strCurLVL,self.tbTopList[nRank][4])
		end
	end
	CreateNewSayEx(strTittle,{{"§ãng",OnCancel}})
end
...
function tbTop10:UpdateTopList()
	...
	nMapID = SubWorldID2Idx(53)
	if nMapID < 0 then
		print("\nMap Ba Lang Huyen khong co tren server nay")
		return
	end
	if (not self.tbSubcribers) then
		print("update danh sach top tan thu rong!")
		return
	end
	if (self.tbSubcribers==nil) then
		print("update danh sach top tan thu rong!")
		return
	end
	...
	for key, val in self.tbSubcribers do
		...
		--nExp = (self.tbCurLvl_Exp[val[self.nCUR_LVL]]) + val[self.nCUR_EXP]
		nExp = (self.tbCurLvl_Exp[val[self.nCUR_LVL]]) + val[self.nCUR_EXP]
		--nExp =GetExp()/1e9

		tinsert(tbTemp, {key, val[self.nCUR_LVL], val[self.nCUR_EXP_PERCENT], nExp, strTime})
	end
	--s¾p xÕp table t¹m theo thø tù gi¶m dÇn dùa ltæng EXP
