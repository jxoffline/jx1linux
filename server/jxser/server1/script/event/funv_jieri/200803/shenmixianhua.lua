...
function funv_jieri_shenmi:xianhua()
	...
	Say("Ch¼ng hay c¸c h¹ muèn tÆng ta hoa g× nhØ?",3,"Ta muèn tÆng bã hoa hång./#funv_jieri_shenmi:zengsongmeigui(1)","Ta muèn tÆng bã hoa cóc./#funv_jieri_shenmi:zengsongjuhua(1)","Tho¸t ra/NoChoice")
end
...
function funv_jieri_shenmi:zengsongmeigui(sel)
	...
	if ntaskmeigui >= tbmeigui.limit then
		Say("Mçi nh©n vËt chØ cã thÓ tÆng cho 2 nh©n vËt nhiÖm vô bİ Èn tèi ®a 25 bã hoa hång.",0)
		return
	end
...
function funv_jieri_shenmi:zengsongjuhua(sel)
	...
	if ntaskjuhua >= tbjuhua.limit then
		Say("Mçi nh©n vËt chØ cã thÓ tÆng cho 2 nh©n vËt nhiÖm vô bİ Èn tèi ®a 125 bã hoa cóc.",0)
		return
	end
