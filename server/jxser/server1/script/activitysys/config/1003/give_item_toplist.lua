...
tbTop10.tbPhase = {
	[1] = {201304010000, 201304292400}, --nép
	[2] = {201304300100, 201304302400}, --nhËn th­ëng
}
...
function tbTop10:GiveItem(strName, strAccount, nCount)
	...
	if self:Subcribers2File() == 1 then
		Say("Nép thµnh c«ng Tói ChiÕn Th¾ng", 1, "§ãng/OnCancel")
	end
end
...
function tbTop10:UpdateTopList()
	...
	--phÇn th­ëng ng­êi ®øng top trong ngµy ®Çu tiªn
...
function tbTop10:IncreaseAwardCounter()
	...
	local tbTemp = {}
	--insert header cña file
	tinsert(tbTemp, {"Sè l­îng ®· nhËn th­ëng"})
...
function tbTop10:CheckCanGetAward()
	if self.nAwardedCount >= 10 then
		Talk(1, "", "TÊt c¶ phÇn th­ëng ®· cã chñ.")
		return 0
	else
		return 1
	end
