...
--§iÒu chØnh kinh m¹ch - by NgaVN 
	...
	[12] = {szName = "HuyÕt Long §»ng", tbProp = {6, 1, 3328,6}, nCount = 10},
...
function MeridianLevelUpCheck(nMeridianIndex, nNewLevel, nNeedZY, nNeedHMD, nZYTaskID,nWay)
	...
	if GetTask(nZYTaskID) < nNeedZY then
		Msg2Player(format("Muèn xung kai huyÖt vÞ nµy cÇn %d ®iÓm Ch©n Nguyªn, Ch©n Nguyªn cña c¸c h¹ kh«ng ®ñ!", nNeedZY));
		return 0;
	end
	--ÐèÒª»¤Âöµ¤ÊýÁ¿
	if CalcItemCount(3, %tbHuMaiDan[1], %tbHuMaiDan[2], %tbHuMaiDan[3], -1) < nNeedHMD then
		Msg2Player(format("Muèn xung kai huyÖt vÞ nµy cÇn %d Hé M¹ch §¬n, Hé M¹ch §¬n cña c¸c h¹ kh«ng ®ñ!", nNeedHMD));
		return 0;
	end
	return 1;
end
...
function MeridianLevelUp(nMeridianIndex, nAcupIndex, nSuccessRate, nRollBackLevel, szMeridianName, szAcupName, nNeedZY, nNeedHMD, nZYTaskID, nWay)
	...
	if random(1, 10000) <= nSuccessRate then
		...
		if bProtection then
			Msg2Player("Do cã HuyÕt Long §»ng b¶o hé, kinh m¹ch sÏ kh«ng bÞ tæn thÊt.")
		end
...
function SetUpSkillIDAndLevel()
	...
	if nSkillID then
		if HaveMagic(nSkillID) ~= nSkillLevel then
			...
			Msg2Player(format("Ng­¬i nhËn ®­îc vâ c«ng thÊt truyÒn cña m«n ph¸i cÊp %d, më trang kÜ n¨ng ra xem. ", nSkillLevel));
		end
	end
end
